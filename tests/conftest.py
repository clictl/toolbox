...
def _spec_requires_auth(spec: dict[str, Any]) -> bool:
    """Check if the spec requires authentication.

    Spec 1.0 format uses auth.env + auth.header (or auth.param).
    Legacy format used auth.type.
    """
    auth = spec.get("auth")
    if auth is None:
        return False
    if isinstance(auth, list):
        return any(_auth_entry_requires(a) for a in auth)
    return _auth_entry_requires(auth)


def _auth_entry_requires(auth: dict[str, Any]) -> bool:
    """Check if a single auth entry requires credentials."""
    # 1.0 format: env + header/param
    if auth.get("env"):
        return True
    if auth.get("header") or auth.get("param"):
        return True
    # Legacy format
