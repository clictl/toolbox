...
One-page cheat sheet for clictl tool specs. See [spec-format.md](spec-format.md) for full details.
...
# Static actions are metadata for search. Tools discovered at runtime.
actions:
  - name: query
...
| `output` | `json` |
| `mutable` | `false` |
| `discover` | auto (stdio=true, http=false) |
| `pricing.model` | `free` |
