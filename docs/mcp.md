...
When `discover: true`, clictl calls `tools/list` on the MCP server at runtime to get the full set of available tools. This is useful for servers that expose many tools or tools that change based on configuration.
...
You can combine discovery with static overrides:
...
1. Static actions in the spec are indexed for search and display
...
| Scenario | Approach |
|----------|----------|
| Server has a small, fixed set of tools | Static only (no `discover`) |
| Server has many tools, you want all of them | `discover: true`, no static |
| Server has many tools, you want better descriptions for key ones | `discover: true` with `static` overrides |
| Server tools change based on config (e.g., database tables) | `discover: true` |
...
Even with `discover: true`, write static definitions for the tools the agent will use most. Better descriptions, examples, and instructions make the agent more effective.
